  ...
  callbacks: {
    ...
    signIn: async ({ account, user, email }) => {
      console.log("[signIn] Starting signIn callback", {
        ...
        isVerificationRequest: email?.verificationRequest,
      });
      ...
      if (user.email && email?.verificationRequest) {
        ...
        if (oneMinRateLimiter && ip) {
          ...
          console.log("[signIn] Rate limit check:", success);
          if (!success) throw new Error("too-many-requests");
        }
        const emailValid = isEmailLegit(user.email);
        console.log(
          "[signIn] isEmailLegit check:",
          emailValid,
          "for email:",
          user.email,
        );
      ...
      console.log(
        "[signIn] All checks passed, calling accountHasRequiredOAuthGroups",
      );
